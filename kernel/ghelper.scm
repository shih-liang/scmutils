...
(define-integrable (guarantee-symbol object procedure)
  (if (not (symbol? object))
      (error:wrong-type-argument object "symbol" procedure)))

(define-integrable (guarantee-procedure-arity object procedure)
  (if (not (procedure-arity? object))
      (error:wrong-type-argument object "procedure arity" procedure)))

(define (make-generic-operator arity #!optional name default-operation)
  ...
  (let ((record (make-operator-record arity)))
    ...
    (if (not (default-object? name))
	(begin (set-operator-record! name record) ; backwards compatibility
               (eq-put! operator 'name name)))    ; debugging
